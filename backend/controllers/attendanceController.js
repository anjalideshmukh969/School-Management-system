  ...
  try {
    ...
    const ops = records.map((r) => ({
      updateOne: {
        ...
        update: { $set: { classRoom, status: r.status, remarks: r.remarks || "", markedBy: teacher._id } },
        upsert: true,
      },
    }));
...
export const getClassAttendance = async (req, res, next) => {
  try {
    ...
    const records = await Attendance.find({ classRoom: classId, date: { $gte: day, $lt: nextDay } }).populate({ path: "student", populate: { path: "user", select: "name" } });
    res.json(records);
  } catch (err) { next(err); }
};
