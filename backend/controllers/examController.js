...
// Bulk marks entry for one exam. Body: { results: [{ student, marksObtained, remarks }] }
      ...
      updateOne: {
        ...
        update: {
          $set: {
            ...
            grade: gradeFor(r.marksObtained),
            remarks: r.remarks || "",
          },
        },
        upsert: true,
...
export const getExamResults = async (req, res, next) => {
  try {
    const results = await Result.find({ exam: req.params.examId })
      ...
      .populate("exam", "title maxMarks passingMarks subject");
    res.json(results);
  } catch (err) {
    next(err);
  }
...
export const getMyResults = async (req, res, next) => {
  try {
    ...
    const results = await Result.find({ student: student._id })
      .populate("exam", "title subject maxMarks passingMarks examDate")
      .sort({ createdAt: -1 });
