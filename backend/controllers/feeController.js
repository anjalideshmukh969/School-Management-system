...
export const createFeeRecord = async (req, res, next) => {
  try { const fee = await Fee.create(req.body); res.status(201).json(fee); } catch (err) { next(err); }
};
export const getStudentFees = async (req, res, next) => {
  try { const fees = await Fee.find({ student: req.params.studentId }).sort({ dueDate: 1 }); res.json(fees); } catch (err) { next(err); }
};
