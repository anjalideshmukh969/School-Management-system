const Table = ({ columns, rows, emptyMessage = "No records found" }) => {
  if (!rows || rows.length === 0) return <p className="text-gray-500 text-sm py-8 text-center">{emptyMessage}</p>;
  return (
    <div className="overflow-x-auto rounded-lg border border-gray-200">
      <table className="min-w-full divide-y divide-gray-200 text-sm">
        <thead className="bg-gray-50">
          <tr>{columns.map((col) => <th key={col.key} className="px-4 py-3 text-left font-semibold text-gray-600">{col.label}</th>)}</tr>
        </thead>
        <tbody className="divide-y divide-gray-100 bg-white">
          {rows.map((row, i) => (
            <tr key={row._id || i} className="hover:bg-gray-50">
              {columns.map((col) => <td key={col.key} className="px-4 py-3 text-gray-700">{col.render ? col.render(row) : row[col.key]}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  );
};
export default Table;
