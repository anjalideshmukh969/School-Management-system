...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    api.get("/dashboard/admin-stats").then(({ data }) => setStats(data)).catch(() => setStats(null)).finally(() => setLoading(false));
  }, []);
  return (
              ...
              <ul className="divide-y divide-gray-100">
                {stats.recentNotices.map((n) => (
                  <li key={n._id} className="py-3">
                    ...
                    <p className="text-xs text-gray-400 mt-1">By {n.postedBy?.name} · {new Date(n.createdAt).toLocaleDateString()}</p>
                  </li>
                ))}
              </ul>
