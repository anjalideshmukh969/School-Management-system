...
const TimetableAdmin = () => {
  ...
  const handleSave = async () => {
    ...
    setMessage("");
    try {
      await api.post("/timetable", { classRoom: selectedClass, day: selectedDay, periods });
      setMessage(`Timetable saved for ${selectedDay}.`);
    } catch (err) {
    ...
    <DashboardLayout title="Timetable">
      <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6 flex flex-wrap gap-4 items-end">
        ...
        <div>
          ...
          <select value={selectedDay} onChange={(e) => setSelectedDay(e.target.value)} className="input w-40">
      ...
      {selectedClass && (
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={handleSave} disabled={saving} className="ml-auto bg-primary-700 hover:bg-primary-800 disabled:opacity-60 text-white text-sm font-medium px-5 py-2 rounded-lg">
