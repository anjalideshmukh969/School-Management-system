...
const Notices = () => {
  ...
  const handleSubmit = async (e) => { e.preventDefault(); setSaving(true); try { await api.post("/notices", form); setForm(emptyForm); setShowForm(false); loadNotices(); } finally { setSaving(false); } };
  const handleDelete = async (id) => { if (!confirm("Delete this notice?")) return; await api.delete(`/notices/${id}`); loadNotices(); };
  return (
    <DashboardLayout title="Notices">
      <div className="flex justify-between items-center mb-4">
        <p className="text-gray-500 text-sm">{notices.length} notices</p>
        <button onClick={() => setShowForm((s) => !s)} className="bg-primary-700 hover:bg-primary-800 text-white text-sm font-medium px-4 py-2 rounded-lg">{showForm ? "Cancel" : "+ New Notice"}</button>
      </div>
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-xl p-5 mb-6 space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="flex items-center gap-2 text-sm text-gray-600"><input type="checkbox" name="isPinned" checked={form.isPinned} onChange={handleChange} />Pin to top</label>
          </div>
          <button type="submit" disabled={saving} className="bg-primary-700 hover:bg-primary-800 disabled:opacity-60 text-white font-medium px-6 py-2.5 rounded-lg">{saving ? "Posting..." : "Post Notice"}</button>
        </form>
      )}
        ...
        {notices.map((n) => (
          <div key={n._id} className="bg-white border border-gray-200 rounded-xl p-5">
            <div className="flex justify-between items-start">
              <div>
                <p className="font-semibold text-gray-800">{n.isPinned && "📌 "}{n.title}<span className="ml-2 text-xs font-normal text-primary-700 bg-primary-50 px-2 py-0.5 rounded-full">{n.audience}</span></p>
