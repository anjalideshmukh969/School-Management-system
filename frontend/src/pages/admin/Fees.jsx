...
const Fees = () => {
  ...
  const loadFees = (studentId) => {
    if (!studentId) return setFees([]);
    api.get(`/fees/student/${studentId}`).then(({ data }) => setFees(data));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await api.post("/fees", { ...form, student: selectedStudent || form.student });
      setForm(emptyForm);
      setShowForm(false);
    ...
    { key: "status", label: "Status", render: (r) => <span className={`text-xs font-medium px-2.5 py-1 rounded-full capitalize ${STATUS_COLORS[r.status]}`}>{r.status}</span> },
    {
      key: "action", label: "Record Payment", render: (r) => r.status !== "paid" && (
        <div className="flex gap-2">
          <input
            type="number" placeholder="Amount" value={payAmount[r._id] || ""}
            onChange={(e) => setPayAmount({ ...payAmount, [r._id]: e.target.value })}
  ...
  return (
    <DashboardLayout title="Fee Management">
      ...
      {selectedStudent ? (
        <Table columns={columns} rows={fees} emptyMessage="No fee records for this student yet." />
      ) : (
        <p className="text-gray-500 text-sm text-center py-8">Select a student to view or add fee records.</p>
      )}
    </DashboardLayout>
  );
};
