...
const Teachers = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm(emptyForm); setShowForm(false); loadTeachers();
    } catch (err) { setError(err.response?.data?.message || "Failed to add teacher"); }
    finally { setSaving(false); }
  };
  const handleDeactivate = async (id) => { if (!confirm("Deactivate this teacher?")) return; await api.delete(`/teachers/${id}`); loadTeachers(); };
  const columns = [
  ...
  return (
    <DashboardLayout title="Teachers">
      <div className="flex justify-between items-center mb-4">
        ...
        <button onClick={() => setShowForm((s) => !s)} className="bg-primary-700 hover:bg-primary-800 text-white text-sm font-medium px-4 py-2 rounded-lg">{showForm ? "Cancel" : "+ Add Teacher"}</button>
      </div>
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-xl p-5 mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <button type="submit" disabled={saving} className="sm:col-span-2 bg-primary-700 hover:bg-primary-800 disabled:opacity-60 text-white font-medium py-2.5 rounded-lg">{saving ? "Saving..." : "Save Teacher"}</button>
        </form>
      )}
