...
const Marks = () => {
  ...
  useEffect(() => {
    ...
    const exam = exams.find((ex) => ex._id === selectedExam);
    if (!exam) return;
    api.get(`/students?classRoom=${exam.classRoom?._id || exam.classRoom}`).then(({ data }) => {
      setStudents(data);
      const initial = {};
  ...
  const handleSubmitMarks = async () => {
    ...
    try {
      const results = students
        .filter((s) => marksMap[s._id] !== "")
        .map((s) => ({ student: s._id, marksObtained: Number(marksMap[s._id]) }));
  ...
  return (
    <DashboardLayout title="Marks Entry">
      <div className="flex justify-between items-center mb-4">
        ...
        <button onClick={() => setShowExamForm((s) => !s)} className="bg-primary-700 hover:bg-primary-800 text-white text-sm font-medium px-4 py-2 rounded-lg">
      ...
      {showExamForm && (
        <form onSubmit={handleCreateExam} className="bg-white border border-gray-200 rounded-xl p-5 mb-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <select name="classRoom" required value={examForm.classRoom} onChange={handleExamChange} className="input">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6 flex flex-wrap items-end gap-4">
        <div>
          ...
          <select value={selectedExam} onChange={(e) => setSelectedExam(e.target.value)} className="input w-72">
            <option value="">Select exam</option>
            {exams.map((ex) => (
              <option key={ex._id} value={ex._id}>{ex.title} · {ex.subject} ({ex.classRoom?.name} {ex.classRoom?.section})</option>
            ))}
          </select>
        </div>
        {students.length > 0 && (
          <button onClick={handleSubmitMarks} disabled={saving} className="bg-primary-700 hover:bg-primary-800 disabled:opacity-60 text-white text-sm font-medium px-5 py-2.5 rounded-lg">
