...
const GRADE_COLORS = {
  ...
  B: "text-primary-700 bg-primary-50", C: "text-amber-700 bg-amber-50",
  D: "text-amber-700 bg-amber-50", F: "text-red-700 bg-red-50",
};
...
const MyResults = () => {
  ...
  useEffect(() => {
    api.get("/exams/results/me").then(({ data }) => setResults(data));
  }, []);
  ...
  const columns = [
    ...
    { key: "marks", label: "Marks", render: (r) => `${r.marksObtained} / ${r.exam?.maxMarks}` },
    {
      key: "grade", label: "Grade", render: (r) => (
        <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${GRADE_COLORS[r.grade] || ""}`}>{r.grade}</span>
      ),
    },
  ...
  return (
    <DashboardLayout title="My Results">
      <Table columns={columns} rows={results} emptyMessage="No results published yet." />
    </DashboardLayout>
  );
};
